    ...
    if (links[nodeId]) {
        ...
        for (var j = 0; j < n; j++) {
            ...
            if (nodes[target]) {
                ...
                var props = _buildLinkProps(source, target, nodes, links, config, linkCallbacks, someNodeHighlighted, transform);
...
function buildGraph(nodes, nodeCallbacks, links, linkCallbacks, config, someNodeHighlighted, transform) {
    ...
    for (var nodeId in nodes) {
        var props = _buildNodeProps(nodes[nodeId], config, nodeCallbacks, someNodeHighlighted, transform);

        nodesComponents.push(_react2.default.createElement(_Node2.default, _extends({ key: nodeId }, props)));

        linksComponents = linksComponents.concat(_buildNodeLinks(nodeId, nodes, links, config, linkCallbacks, someNodeHighlighted, transform));
    }
...
function createForceSimulation(width, height) {
    ...
    var simulation = (0, _d3Force.forceSimulation)().force('charge', (0, _d3Force.forceManyBody)().strength(_const2.default.FORCE_IDEAL_STRENGTH)).force('x', frx).force('y', fry);
